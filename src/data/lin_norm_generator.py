...
class LinearNormalDataGenerator(DataGenerator):
    ...
    def __init__(
        ...
        iv_strength_range: list = [0.1, 1.0],
        conf_strength_range: list = [0.1, 1.0],
        treat_effect_range: list = [-1.0, 1.0],
        conf_effect_range: list = [1.0, 5.0],
        error_scale: float = 1.0,
        beta_range: list = [-1.0, 1.0],
        base_seed: int = 42,
    ) -> None:
        """
    ...
    def generate(self) -> dict:
        ...
        iv_strength = np.random.uniform(
            self.iv_strength_range[0], self.iv_strength_range[1]
        )
        conf_strength = np.random.uniform(
            self.conf_strength_range[0], self.conf_strength_range[1]
        )
        conf_effect = np.random.uniform(
            self.conf_effect_range[0], self.conf_effect_range[1]
        )
        treat_effect = np.random.uniform(
            self.treat_effect_range[0], self.treat_effect_range[1]
        )
        ...
        t = (
            ...
            + iv_strength * z
            + conf_strength * c
            + self.error_scale * np.random.normal(0, 1, size=n_samples)
        )
